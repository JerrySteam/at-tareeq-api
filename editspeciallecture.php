<?php
  ...
  if ($title === "" ||
    ...
    if (isset($_POST['photo'])){
      ...
      echo updateLectureInfo($lectureid, $title, $speaker, $location, $date, $time, $briefinfo, $photourl);
    }else{
  ...
  function updateLectureInfo($lectureid, $title, $speaker, $location, $date, $time, $briefinfo, $photourl){
    $categoryid = 2;
    try{
      global $dbh;
      ...
      if($rs > 0){
        $res = updateLectureDayTime($lectureid, $date, $time);
        if ($res['success']) {
          return outputInJSON(true, "Special lecture successfully updated");
        }else{
          return outputInJSON(false, $res['message']);
        }
      }else{
        return outputInJSON(false, "Error. Special lecture not updated");
      }
    } catch (Exception $e) {
  ...
  function updateLectureDayTime($lectureid, $date, $time){
    try{
      global $dbh;
      $cArray = array('time'=>$time, 'dayordate'=>$date);
      $wArray = array('lectureid'=>$lectureid);
