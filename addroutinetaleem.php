<?php
  ...
  if ($title === "" ||
    ...
    if (isset($_POST['photo'])){
      ...
      echo saveLectureInfo($userid, $title, $speaker, $location, $weekday, $time, $briefinfo, $latitude, $longitude, $photourl);
    }else{
  ...
  function saveLectureInfo($userid, $title, $speaker, $location, $weekday, $time, $briefinfo, $latitude, $longitude, $photourl){
    $categoryid = 1;
    try{
      global $dbh;
      ...
      if($lectureid > 0){
        $res = saveLectureDayTime($lectureid, $weekday, $time);
        if ($res['success']) {
